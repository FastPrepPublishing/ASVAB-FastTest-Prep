# app.py – ASVAB Exam Prep by FastPrep Publishing (Funzioni avanzate)
import pandas as pd
...
# Opzioni di risposta
user_answer = st.radio("Choose your answer:", [q["Option A"], q["Option B"], q["Option C"], q["Option D"]],
                       index=-1 if st.session_state.current_q not in st.session_state.answers else
                       ["A", "B", "C", "D"].index(st.session_state.answers[st.session_state.current_q]),
                       key=f"question_{st.session_state.current_q}")

# Mappatura risposta selezionata
if user_answer:
    for label, text in zip(["A", "B", "C", "D"], [q["Option A"], q["Option B"], q["Option C"], q["Option D"]]):
        if user_answer == text:
