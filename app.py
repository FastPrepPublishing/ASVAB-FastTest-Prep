...
def practice_mode():
    ...
    answer = st.radio("Scegli una risposta:", options, key=f"practice_{st.session_state.practice_index}")
...
def exam_mode():
    ...
    if st.session_state.exam_index < len(df):
        ...
        st.markdown(f"**{q['Question']}**")
        options = [q["Option A"], q["Option B"], q["Option C"], q["Option D"]]
        answer = st.radio("Scegli una risposta:", options, key=f"exam_{st.session_state.exam_index}")
        if st.button("Conferma risposta"):
            correct = q[f"Option {q['Correct Answer']}"]
            if answer == correct:
                st.session_state.score += 1
            st.session_state.answers.append((q['Question'], answer, correct))
