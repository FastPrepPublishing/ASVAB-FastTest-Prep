...
def exam_mode():
    ...
    if st.session_state.exam_index < len(df):
        ...
        st.markdown(f"**{q['Question']}**")
        options = [q["Option A"], q["Option B"], q["Option C"], q["Option D"]]
        selected = st.radio("Scegli una risposta:", options, key=f"exam_q{st.session_state.exam_index}")
        ...
        if not st.session_state.answer_submitted:
            if st.button("Conferma risposta"):
                ...
                st.session_state.answers.append((q['Question'], selected, correct))
                st.session_state.answer_submitted = True
                st.experimental_rerun()
        else:
            st.success("Risposta registrata. Avanti...")
